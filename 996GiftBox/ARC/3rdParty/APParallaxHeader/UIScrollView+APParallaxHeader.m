...
static char UIViewTransparentView;
...
//        [view setClipsToBounds:YES];
        ...
        /////////////////////////////////////////
        APParallaxView *contentView = [[APParallaxView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, height)];
        contentView.backgroundColor = [UIColor clearColor];
        [contentView setClipsToBounds:YES];
        
        contentView.scrollView = self;
        contentView.parallaxHeight = height;
        [self addSubview:contentView];
        
        contentView.originalTopInset = self.contentInset.top;
        
        self.transparentView = contentView;
...
- (void)printSubView:(UIView *)view
{
    for (UIView *subView in [view subviews]) {
        NSLog(@"subView = %@---tag = %d",subView,subView.tag);
...
- (void)setTransparentView:(APParallaxView *)transparentView {
    objc_setAssociatedObject(self, &UIViewTransparentView,
                             transparentView,
                             OBJC_ASSOCIATION_ASSIGN);
}

- (APParallaxView *)transparentView {
    return objc_getAssociatedObject(self, &UIViewTransparentView);
}
...
- (void)setShowsParallax:(BOOL)showsParallax {
    ...
    self.transparentView.hidden = !showsParallax;
    ...
    if(!showsParallax) {
        ...
        if (self.transparentView.isObserving) {
            [self removeObserver:self.transparentView forKeyPath:@"contentOffset"];
            [self removeObserver:self.transparentView forKeyPath:@"frame"];
            self.transparentView.isObserving = NO;
        }
    }
    else {
        if (!self.parallaxView.isObserving) {
        ...
        if (!self.transparentView.isObserving) {
            [self addObserver:self.transparentView forKeyPath:@"contentOffset" options:NSKeyValueObservingOptionNew context:nil];
            [self addObserver:self.transparentView forKeyPath:@"frame" options:NSKeyValueObservingOptionNew context:nil];
            self.transparentView.isObserving = YES;
        }
    }
}
