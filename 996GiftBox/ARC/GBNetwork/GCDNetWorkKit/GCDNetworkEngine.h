...
@interface GCDNetworkEngine : NSObject

/*
 * @abstrach 初始化network engine with a hostname
 *
 *
 * @discussion
 * 通过给定的hostName 创建一个engine 
 * hostName可选
 * 如果hostName不为空，将进行网络可达检查
 *
 **/
 ...
 */
/*
 * @abstrach 加入operationBlock到并发队列
 *
 *
 * @discussion
 * 若request支持缓存，且存在缓存数据，将返回缓存数据
 *
 **/
 ...
 *
 * @discussion
 * 若forceReload ＝ NO 同enqueueOperation
 * 若forceReload = YES 将请求最新数据
 **/
...
@property (nonatomic, strong) NSNumber* operationsCount;
